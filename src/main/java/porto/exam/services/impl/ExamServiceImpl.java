...
public class ExamServiceImpl implements ExamService {
    ...
    public void upsert(UpsertDto dto) {
        if (ZonedDateTime.now(ZoneId.of("UTC")).isAfter(dto.getStartDate()))
            throw new BadLogicException("Exam has started, unable to make changes");
        ...
        for (var question : dto.getQuestions()) {
            ...
            if (!question.isNew()) questionEntity.setText(question.getText());
            ...
            if (multipleCorrectAnswer)
                throw new BadLogicException("Multiple correct answers in question: " + question.getText());
        ...
        for (var entityToDelete : dto.getFormDelete()) {
            ...
            if (entityToDelete.getType() == DeleteType.ANSWER)
                answerRepository.deleteById(entityToDelete.getEntityId());
        }
    }
    ...
    public void submit(SubmitDto dto) {
        ...
        if (entity.getSubmitDate() != null || entity.getGrade() != null)
            throw new BadLogicException("Finalized; can't make changes");
    ...
    public ExamListDto fetchExamWithQnA(String examId, String studentId, String timezone) {
        ...
        for (var groupedExam : groupedQuestions.entrySet()) {
            ...
            for (var groupedQuestion : groupedAnswers.entrySet()) {
                var questionId = groupedQuestion.getValue().getFirst().getQuestionId();
                var text = groupedQuestion.getValue().getFirst().getQuestionText();
                var correct = groupedQuestion.getValue().stream()
                        .filter(FlatExamQnADto::getIsCorrect)
                        .map(FlatExamQnADto::getAnswerId)
    ...
    public void grade(String examId) {
        ...
        if (exam.getEndDate().isAfter(ZonedDateTime.now())) throw new BadLogicException("Exam has not ended");
        ...
        for (var studentExam : studentExams) {
            ...
            var grade = correct > 0 ? (int) Math.ceil(((float) correct / correctAnswers.size()) * 100) : 0;
    ...
    public GradeDto fetchGradeData(String courseTeacherId, String examId) {
        ...
        for (var groupedStudent : groupedStudents.entrySet()) {
            ...
            var passRate = (!students.isEmpty())
                    ? passAmt.get() / students.size()
                    : 0;
            var avg = (!students.isEmpty() && studentWithGrade.get() > 0)
                    ? total.get() / studentWithGrade.get()
                    : 0;
